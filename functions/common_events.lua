        ...
        if not card.unlocked and card.unlock_condition and card.unlock_condition.type == args.type then
            ...
            if args.type == 'play_all_hearts' then
                ...
                for k, v in ipairs(G.deck.cards) do
                    if not v.ability.config.no_rank_suit and v.base.suit == 'Hearts' then 
                        played = false
                    end
                end
...
function get_current_pool(_type, _rarity, _legendary, _append)
        ...
        if _type == 'Joker' then 
            ...
            if (#find_joker('Connoisseur') > 0) then rarity = _rarity or 0.8 end  -- mod
...
function reset_idol_card()
    ...
    for k, v in ipairs(G.playing_cards) do
        if not v.ability.config.no_rank_suit then
...
function generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end)
    ...
    elseif _c.set == 'Enhanced' then 
        ...
        localize{type = 'descriptions', key = _c.key, set = _c.set, nodes = desc_nodes, vars = loc_vars}
        if not _c.ability.config.no_rank_suit and ((specific_vars and specific_vars.bonus_chips) or _c.config.bonus) then
    ...
    elseif _c.set == 'Tarot' then
       ...
       elseif _c.name == "A Critic" then loc_vars = {_c.config.planets} 
